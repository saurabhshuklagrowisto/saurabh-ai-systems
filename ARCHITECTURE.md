...
Lives in [shared/guardrails.py](./shared/guardrails.py). Three things happen before any Claude call:
...
Every call tags the output with the prompt version. Production logs let you ask: "show me all outputs from prompt v2 with confidence < 7 in the last 24 hours" — that question is unanswerable if prompts are buried in code or rolled into a system message constant.
...
- **Length compliance** — subject ≤ 50 chars, body ≤ 60 words
...
In the demos, this stage is collapsed (we print to stdout). In production via n8n, see [02-reply-triage-agent/n8n-workflow.json](./02-reply-triage-agent/n8n-workflow.json) for the full switch graph.
...
This is where most teams stop. They build stages 1–4 and call it shipped.

The eval loop is what makes the system **continue to work** as prompts change, models update, and the world drifts.

See [02-reply-triage-agent/eval.py](./02-reply-triage-agent/eval.py). The pattern:
...
**Composability.** When you add a fifth workflow, you reuse `shared/guardrails.py`. Without the separation, you copy-paste the PII regex into every new agent. The first time the regex needs to change for a compliance update, you have a 14-file diff.
...
- There's no production scale yet (under 50 calls/day) — Stage 5 can be a weekly batch eval instead of a CI gate
