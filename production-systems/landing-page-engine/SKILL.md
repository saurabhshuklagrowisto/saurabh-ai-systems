...
1. **Section structure** is an ordered list of page sections
2. **Copy per section** is finished, on-brand copy for each section (headline, sub-head, body, CTA)
3. **Widget map** says which Elementor widget holds each piece of content, so a non-technical teammate can assemble it
4. **Form connection note** says where the registration form embeds and what fields it needs

### Path B · `custom_html`

1. **One self-contained HTML file** with inline CSS, mobile-first responsive, no external dependencies
...
All generated from the same `brief` plus `brand_kit` so the campaign is visually coherent.
