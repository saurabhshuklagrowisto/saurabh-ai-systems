...
<table width="100%">
<tr>
<td align="center" width="500"><b>16</b><br>production systems live</td>
...
<td align="center" width="500"><b>25+</b><br>qualified meetings / month</td>
<td align="center" width="500"><b>6%+</b><br>reply rate on cold outbound</td>
</tr>
</table>
...
| [**Email Verification Gate**](./production-systems/email-verification-gate) | A hard gate between a lead pipeline and the CRM: no address reaches Close CRM or SalesBlink without a verification verdict, catch-all and unknown are held rather than pushed, and an unrecognised status holds too, so a new verdict word can never fail open. The verdict cache is the product, not an optimisation: it is what stops a weekly re-scrape re-billing the entire base, and the credit rule is enforced by a test that fails the build if a cached address is ever re-sent. A held email drops the address, never the person. | ![Live](https://img.shields.io/badge/status-live-success?style=flat-square) |
...
| [AEO / GEO Improvement Bot](./claude-skills/aeo-geo-improvement-bot) | Closes the loop on the audit above: diagnoses why each gap question is invisible (content, schema, citations, or authority) and returns a ranked, priority-ordered fix plan | `python claude-skills/aeo-geo-improvement-bot/scripts/geo_improve.py` |
