# 02 · Reply Triage Agent

Classifies inbound replies to cold outreach and decides routing: auto-draft + human approval, full human review, CRM update only, or suppression. Ships with a golden-set eval that blocks promote when critical safety rules are violated.
...
- **Critical-safety rules** — zero-tolerance gates that override any threshold scoring (never auto-reply to unsubscribe, never auto-reply at confidence < 7)
- **Regression-gated eval** — golden set + 5 metrics + CI exit code
...
| reply-006 | auto_reply at confidence 6 < 7 | Violates the explicit prompt rule |
...
├── golden_set.json           ← human-labeled correct answers + critical safety rules
...
└── n8n-workflow.json         ← importable n8n workflow (Smartlead → Claude → Zoho)
```

## The production version

In production at Growisto, this runs in n8n with Smartlead webhooks → Claude classifier → switch node → Zoho CRM update + Slack review queue. The Python script in this repo is the same logic, runnable standalone, so the prompts and guardrails can be developed and tested without the full integration stack.
