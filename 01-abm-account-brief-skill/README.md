...
This is the packaged, standalone version of a workflow that runs in production via n8n + Zoho Flow + Claude API for outbound at Growisto.

---

## What it does

**Input:** target account domain + persona + recent signals.
...
- **Prompt versioning** — v1 → v2 with documented reason (v1 failed hook diversity 3/10 cases on generic-growth hooks)
...
Eval scores on: schema validity (binary), evidence grounding (every hook cites a URL from input signals), hook diversity (no two hooks paraphrase the same signal), length compliance (subject ≤ 50, body ≤ 60 words), LLM-as-judge rating vs golden hook.
...
- Wire the LLM-as-judge call (currently stubbed for local dev)
- Add an MCP server wrapper so this Skill is callable from Claude Desktop
- Add a HubSpot/Salesforce connector for account history (currently takes it as input)
- Shadow-mode harness — score v(N+1) against v(N) on live traffic before promote
